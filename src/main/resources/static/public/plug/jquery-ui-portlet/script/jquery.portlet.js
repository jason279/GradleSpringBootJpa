    ...
    $.widget("ui.portlet", {
        ...
        _createSinglePortlet: function(portlet, _ele, column, positon, pattrs) {
            ...
            if (positon === 'last') {
				...
				if($(column).find('.ui-portlet-item').length == 0) {
					item.appendTo(column);
				} else {
					if (positon.x === 'last') {
		...
		getWidths: function() {
			var widths = [];
			$('.ui-portlet-column', this.element).each(function(i, v) {
        ...
        _addSingle: function(option) {
            ...
            var addOpt = option;
            var column;
			if(addOpt.position == null) {
				addOpt.position = {
					x: 'last',
					y: $('.ui-portlet-column', _ele).length - 1
				};
			}
