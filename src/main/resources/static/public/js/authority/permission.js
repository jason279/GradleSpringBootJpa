...
$(function() {
   //获取权限列表并生成树
   $.get("/permission_all_json", function(data) {
   ...
   $('#userPermission_add_form').bootstrapValidator({
      ...
      fields: {
          ...
          value: {validators: {callback: {callback: function(value, validator) {return true;}}}}
        	...
        	$.post("/permission_add", $("#userPermission_add_form").serialize(), function(data) {
            if (data.status == 0) {
                var newNode = {
                    isParent : false,
                    id: data.js.id,
   ...
   $('#userPermission_up_form').bootstrapValidator({
      ...
      fields: {
       ...
       $.post("/permission_up", $("#userPermission_up_form").serialize(), function (data) {
           if (data.status == 0) {
   ...
   function userPermission_changeNodes() {
       ...
       var params = {"str": str};
       if(str != "") {
           $.post("/permission_oc_up", params, function(data) {
               if (data.status == 0) {
